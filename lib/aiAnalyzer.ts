...
const ISSUE_RULES: {
  ...
  {
    keywords: ['pothole', 'road damage', 'road kharab', 'sadak', 'crack', 'broken road', 'gaddha', 'road repair', 'damaged road', 'road khali', 'gaddhe', 'road mein gaddha', 'potholes', 'road surface'],
...
const RISK_ESCALATORS = [
  ...
  'critical', 'khatarnak', 'tehlik', 'life threat', 'serious', 'immediate', 'immediate attention',
...
const HINDI_INDICATORS = [
  ...
  'gaya', 'uthaya', 'kharab', 'bahut', 'me', 'ka', 'ki', 'ne', 'ko', 'par', 'tak',
...
function translateSummary(text: string, issueType: string, lang: 'hi' | 'en'): string {
  if (lang === 'hi') {
    ...
    if (issueType === 'Traffic & Road Safety') return 'Traffic signal ya road safety ka issue hai jo movement affect kar raha hai';
